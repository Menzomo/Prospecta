...
- [ ] **Testar e confirmar a assinatura Ed25519 dos webhooks Telnyx em produção** (achado em auditoria de segurança, 2026-07-29): `validateWebhookSignature` (`telnyxProvider.ts`) checava os headers `telnyx-signature-ed25519-signature`/`telnyx-signature-ed25519-timestamp` — nomes errados; os reais são `telnyx-signature-ed25519`/`telnyx-timestamp`. Como consequência, a assinatura de toda requisição da Telnyx (mesmo as legítimas) nunca era encontrada, e o código sempre caía no caminho de fallback mais fraco (checagem de `ConnectionId`, ou pior, aceitar sem checar nada quando nem `ConnectionId` vem no payload) — na prática, qualquer pessoa na internet conseguia forjar webhooks de chamada (criar `calls` em nome de qualquer usuário, debitar a carteira dele) sem autenticação nenhuma. Nome dos headers corrigido. **Falta**: fazer uma ligação real e confirmar nos logs que `telnyx-signature-ed25519` chega presente e que a validação passa a rejeitar de fato uma requisição forjada sem esse header — e decidir um plano B (ex.: segredo embutido na própria URL do webhook) pra qualquer tipo de callback que a Telnyx realmente não assine.
- [x] **Testar e2e o encaminhamento de chamada de entrada** — validado com ligação real em 2026-07-29. No caminho, corrigidos 3 bugs reais que impediam o encaminhamento de funcionar: (1) `validateWebhookSignature` só aceitava um único `ConnectionId` (`TELNYX_APP_ID`), mas a Credential Connection do SDK de saída e a TeXML Application dos números do pool são recursos Telnyx **distintos** com IDs diferentes — passou a aceitar os dois (`TELNYX_APP_ID` + `TELNYX_TEXML_APP_ID`, nova env var); (2) o link "Telefonia" estava oculto do menu desde 11/07 (`hideTelefonia`), impedindo o usuário de chegar no formulário de CPF/CNPJ + celular de encaminhamento — removido, e dashboard agora força redirect pra lá se número atribuído sem cadastro completo; (3) `<Dial>` do encaminhamento não tinha `callerId`, então a Telnyx tentava originar a segunda perna com a identidade do lead (não pertencente à conta) e a operadora recusava a rota (erro 10010 "no route found") — corrigido pra usar o número Prospecta (dono legítimo, CPF/CNPJ registrado pra ANATEL).

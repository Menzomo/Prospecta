...
create index lead_categories_slug_idx on public.lead_categories (slug);


-- ============================================================
-- 4. SEED — categorias iniciais
-- ============================================================

insert into public.lead_categories (name, slug, search_terms) values
...
create table public.global_leads (
  id                   uuid        primary key default gen_random_uuid(),
...
-- usuários autenticados veem somente leads ativos
...
create index global_leads_category_id_idx        on public.global_leads (category_id);
create index global_leads_status_idx             on public.global_leads (status);
create index global_leads_provider_source_idx    on public.global_leads (provider_source);
create index global_leads_provider_external_idx  on public.global_leads (provider_external_id)
...
create index user_leads_user_id_idx        on public.user_leads (user_id);
create index user_leads_global_lead_id_idx on public.user_leads (global_lead_id);
create index user_leads_user_status_idx    on public.user_leads (user_id, status);
create index user_leads_hidden_idx         on public.user_leads (user_id, hidden)
  where hidden = false;

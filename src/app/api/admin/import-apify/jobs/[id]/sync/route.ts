...
export async function POST(_request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    if (!runRes.ok) {
      return NextResponse.json({ error: `Apify retornou ${runRes.status} ao consultar run` }, { status: 502 })
    }
    const runData = await runRes.json() as { data?: { status?: string; defaultDatasetId?: string } }
    runStatus = runData.data?.status ?? 'UNKNOWN'
    if (runData.data?.defaultDatasetId) defaultDatasetId = runData.data.defaultDatasetId

    console.log(`[import-apify/sync] job ${id}, run ${job.apify_run_id}, status: ${runStatus}`)
  } catch (err) {
  ...
  if (runStatus !== 'SUCCEEDED') {
    return NextResponse.json({ message: `Status inesperado: ${runStatus}`, job })
  }
  ...
  try {
    const itemsRes = await fetch(
      `https://api.apify.com/v2/datasets/${defaultDatasetId}/items?token=${token}&limit=${job.requested_limit}`
    )
    if (!itemsRes.ok) {
      ...
      return NextResponse.json({ error: 'Falha ao buscar dataset' }, { status: 502 })
    }
